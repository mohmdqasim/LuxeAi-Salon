        ...
        <div className="grid lg:grid-cols-2 gap-16 items-center">
          ...
          <div className="space-y-12">
            <ScrollReveal animation="fade-in-up">
              <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-accent/10 text-accent text-[10px] font-black uppercase tracking-widest mb-6 border border-accent/20">
            ...
            <ScrollReveal animation="fade-in-up" delay={200} className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              {[
                ...
                { id: 'sms' as Channel, icon: Phone, label: 'Business SMS', color: 'hover:border-accent hover:text-accent' },
              ].map((channel) => (
                <button
                  key={channel.id}
                  onClick={() => {
                  ...
                  className={`p-6 rounded-[2rem] border-2 transition-all flex flex-col items-center gap-3 group ${
                    ...
                    ? 'bg-white dark:bg-slate-900 border-accent shadow-xl shadow-accent/5' 
                ...
                >
                  ...
                  <span className={`text-xs font-black uppercase tracking-widest ${activeChannel === channel.id ? 'text-accent' : 'text-slate-500'}`}>{channel.label}</span>
                  {activeChannel === channel.id && (
          ...
          <div className="relative flex justify-center">
            <ScrollReveal animation="zoom-in" delay={400}>
              <div className="relative w-[320px] h-[640px] bg-primary rounded-[3rem] border-[8px] border-primary shadow-[0_50px_100px_-20px_rgba(0,0,0,0.5)] overflow-hidden">
                {/* Phone Header */}
                <div className={`px-4 pt-10 pb-4 flex items-center gap-3 border-b border-white/10 ${
                  ...
                  <div className="flex-1">
                    <p className="text-sm font-black tracking-tight">Glow Bar AI</p>
                    <p className="text-[10px] opacity-70">GlamBot Assistant</p>
                  </div>
                ...
                <div 
                  ref={scrollRef}
                  className={`h-[450px] overflow-y-auto p-4 space-y-4 scroll-smooth bg-background-light dark:bg-[#0a0a1f]`}
                >
                  {messages.map((msg) => (
                    <div 
                      key={msg.id} 
                    ...
                    >
                      <div className={`max-w-[85%] p-3 text-sm font-medium ${
                        msg.sender === 'user' 
                          ...
                          : 'bg-white dark:bg-slate-800 text-primary dark:text-white rounded-2xl rounded-tl-none shadow-sm'
                      }`}>
                        {msg.text}
                      </div>
                      <div className="flex items-center gap-1 mt-1">
                        ...
                        {msg.sender === 'bot' && <CheckCheck className="w-2 h-2 text-accent" />}
                      </div>
                    </div>
                  ))}
                  {isTyping && (
                    <div className="flex items-center gap-2 p-3 bg-white dark:bg-slate-800 rounded-2xl rounded-tl-none w-16 animate-pulse">
                ...
                <div className="absolute bottom-0 left-0 right-0 p-4 bg-white dark:bg-slate-900 border-t border-white/10">
                  <div className="relative">
                    ...
                    <button 
                      onClick={handleSend}
                      disabled={isTyping}
                      className="absolute right-2 top-1/2 -translate-y-1/2 w-8 h-8 bg-accent rounded-full flex items-center justify-center text-white hover:bg-primary transition-transform disabled:opacity-50"
                    >
                      <Send className="w-4 h-4" />
                    </button>
                  </div>
                  <p className="text-[9px] text-center mt-3 text-slate-400 font-bold uppercase tracking-widest">Premium Salon Assistant</p>
                </div>
              </div>

              {/* Decorative elements around phone */}
              <div className="absolute -top-10 -right-10 w-24 h-24 bg-accent/20 rounded-full blur-2xl animate-pulse"></div>
              <div className="absolute -bottom-10 -left-10 w-32 h-32 bg-primary/10 rounded-full blur-3xl animate-pulse animation-delay-2000"></div>
            </ScrollReveal>
          </div>
