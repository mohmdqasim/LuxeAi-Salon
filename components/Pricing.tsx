      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <ScrollReveal animation="fade-in-up" className="text-center mb-16">
          ...
          <div className="flex items-center justify-center gap-4 mt-10">
            ...
            <div className="flex items-center gap-2">
              <span className={`text-sm font-bold ${isAnnual ? 'text-slate-900 dark:text-white' : 'text-slate-400'}`}>Annual</span>
              <span className="px-2 py-0.5 rounded-md bg-emerald-500/10 text-emerald-500 text-[10px] font-black uppercase tracking-wider">Save 20%</span>
            </div>
          </div>
        </ScrollReveal>
              ...
              <div className="flex-1 bg-white dark:bg-[#0a0a1f] rounded-[2.4rem] p-10 flex flex-col h-full">
                <div className="mb-8">
                  <div className={`w-12 h-12 rounded-2xl flex items-center justify-center mb-6 ${
                    plan.highlight ? 'bg-primary text-white' : 'bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400'
                  }`}>
                ...
                <div className="mb-10">
                  {plan.monthlyPrice !== null ? (
                    ...
                    <div className="text-4xl font-black tracking-tighter">Custom</div>
                  )}
                  {plan.monthlyPrice !== null && (
                ...
                <div className="space-y-4 mt-auto">
                  ...
                  {plan.features.map((feature, i) => (
                    <div key={i} className="flex items-start gap-3">
                      <div className={`mt-1 p-0.5 rounded-full ${plan.highlight ? 'bg-primary/20 text-primary' : 'bg-emerald-500/10 text-emerald-500'}`}>
        ...
        <ScrollReveal animation="fade-in-up" delay={600} className="mt-24 text-center">
          <p className="text-slate-500 dark:text-slate-400 text-sm font-medium">
            Need something different? <a href="#" className="text-primary font-bold hover:underline">Chat with our sales team</a> for a custom volume package.
          </p>
        </ScrollReveal>
      </div>
    </section>
